...
export default function VotingApp() {
    ...
    const castVote = async (idKandidat) => {
        ...
        // memanggil fungsi vote() dari abi
        const tx = await contract.vote(idKandidat);
        await tx.wait();

        alert("Voting berhasil!");
        loadCandidates(); // refresh data otomatis setelah vote berhasil

        } catch (error) {
        console.error(error);
        // menangkap pesan error spesifik dari smart contract
            ...
            {isLoading ? (
                ...
                {candidates.map((kandidat) => (
                    <div key={kandidat.id} className="flex justify-between items-center p-4 border rounded shadow-sm">
                        {/* Isi blok ini tetap sama seperti kode aslimu */}
                        <div>
